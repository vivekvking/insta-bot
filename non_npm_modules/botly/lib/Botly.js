...
const FB_MESSENGER_URL = (FB_URL) => `${FB_URL}me/messages`;
...
Botly.prototype.getVerifySignature = function (secret) {
    return (req, res, buf) => {
        const expected = req.headers['x-hub-signature'];
        const hmac = crypto.createHmac('sha1', secret).update(buf, 'utf-8').digest('hex');
...
Botly.prototype.removePersistentMenu = function (options, callback) {
   const PAGE_URL = `${this.FB_URL}${options.pageId}/messenger_profile`;
   request.delete(
        {
            url: PAGE_URL,
            qs: {
...
Botly.prototype.upload = function (options, callback) {
    const PAGE_URL = `${this.FB_URL}${options.pageId}/message_attachments`;
...
Botly.prototype.send = function (options, callback) {
    ...
    request.post(
        {
            url: FB_MESSENGER_URL(this.FB_URL),
            json: true,
            qs: {
                access_token: this.accessToken || options.accessToken
            },
            ...
            this.emit('sent', options.id, options.message, body);
        });
};
